...
describe('Style Consistency & Regression', () => {
  ...
  it('should have unified mobile styles (< 640px) for chips and contact values', () => {
    ...
    const mobileBlockMatch = cssContent.match(
      /@media\s*\(max-width:\s*640px\)\s*{([\s\S]*?)}\s*\n\s*\.section-wrap/
    );
  ...
  it('should use ellipsis for overflow in both chips and contact values', () => {
    ...
    const contactEllipsis = cssContent.match(
      /\.contact\s+\.value\s*{[^}]*text-overflow:\s*ellipsis/
    );
  ...
  it('should enable 2-column contact grid starting at 540px (Protocolo L)', () => {
    const tabletBreakpoint = cssContent.match(
      /@media\s*\(min-width:\s*540px\)\s*{\s*\.contact-grid\s*{\s*grid-template-columns:\s*repeat\(2,\s*1fr\)/
    );
